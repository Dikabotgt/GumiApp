...
const CalculatorDetailScreen = ({ navigation, route }) => {
  ...
  const [values, setValues] = useState(() => {
    ...
    fields.forEach((f) => {
      if (f.key === "balance")
        initial[f.key] = settings.accountBalance.toString();
      else if (f.key === "leverage")
        initial[f.key] = settings.leverage.toString();
      else if (f.key === "riskPercent")
        initial[f.key] = settings.defaultRiskPercent.toString();
  ...
  const handleCalculate = () => {
    ...
    fields.forEach((f) => {
      if (f.type === "decimal") params[f.key] = parseFloat(values[f.key]) || 0;
        ...
        params[f.key] = parseInt(values[f.key]) || 0;
    ...
    switch (calculator.id) {
      case "lotSize":
        calcResult = calculateLotSize(params);
        break;
      case "positionSize":
        calcResult = calculatePositionSize(params);
        break;
      case "risk":
        calcResult = calculateRisk(params);
        break;
      case "riskPercent":
        calcResult = calculateRiskPercentage(params);
        break;
      case "pipValue":
        calcResult = calculatePipValue(params);
        break;
      case "margin":
        calcResult = calculateMargin(params);
        break;
      case "profit":
        calcResult = calculateProfit(params);
        break;
      case "drawdown":
        calcResult = calculateDrawdown(params);
        break;
      case "compound":
        calcResult = calculateCompound(params);
        break;
      case "riskReward":
        calcResult = calculateRiskReward(params);
        break;
      default:
        calcResult = {};
    }
  ...
  return (
    <GradientBackground style={styles.container}>
      <Header
        ...
        titleStyle={{ fontFamily: fontFamily.serif, fontSize: 20 }}
        showBack
                    ...
                    <View style={styles.directionRow}>
                      {["BUY", "SELL"].map((dir) => (
                        <Pressable
                          key={dir}
                          onPress={() =>
                            setValues((prev) => ({ ...prev, [field.key]: dir }))
                          }
                          style={[
                            styles.directionBtn,
                            {
                              backgroundColor:
                                values[field.key] === dir
                                  ? dir === "BUY"
                                    ? colors.profitLight
                                    : colors.lossLight
                                  : colors.card,
